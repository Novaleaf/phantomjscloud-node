...
var AutoscaleConsumer = (function () {
    ...
    AutoscaleConsumer.prototype._workerLoop = function (idleMs) {
        ...
        Promise.try(function () {
            debugLog("AUTOSCALECONSUMER._workerLoop() starting request processing (workProcessor) concurrent=" + _this._workerCount);
            return _this._workProcessor(work.input);
        }).then(function (output) {
            debugLog("AUTOSCALECONSUMER._workerLoop() finished workProcessor() SUCCESS. concurrent=" + _this._workerCount);
            work.resolve(output);
        }, function (error) {
            debugLog("AUTOSCALECONSUMER._workerLoop() finished workProcessor() ERROR. concurrent=" + _this._workerCount);
            work.reject(error);
        }).finally(function () {
