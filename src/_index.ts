...
export class BrowserApi {
	...
	requestSingle( request: ioDatatypes.IUserRequest | ioDatatypes.IPageRequest, customOptionsOrCallback?: ( ( err?: Error, result?: ioDatatypes.IUserResponse ) => void ) | IBrowserApiOptions, callback?: ( err?: Error, result?: ioDatatypes.IUserResponse ) => void ): PromiseLike<ioDatatypes.IUserResponse> {

		let customOptions: IBrowserApiOptions = customOptionsOrCallback as any;
		return Promise.try( () => {
	...
	public requestBatch( requests: (
		...
		callback?: ( err: Error, item: { request: ( ioDatatypes.IUserRequest | ioDatatypes.IPageRequest ); result: ioDatatypes.IUserResponse } ) => void ): PromiseLike<ioDatatypes.IUserResponse>[];
	public requestBatch(
		requests: ( ioDatatypes.IUserRequest | ioDatatypes.IPageRequest )[],
		customOptions?: IBrowserApiOptions, callback?: ( err?: Error, item?: { request: ( ioDatatypes.IUserRequest | ioDatatypes.IPageRequest ); result?: ioDatatypes.IUserResponse } ) => void ): PromiseLike<ioDatatypes.IUserResponse>[];
	requestBatch(
		requests: ( ioDatatypes.IUserRequest | ioDatatypes.IPageRequest )[],
		...
		describe( "success cases", () => {
			describe( "basic browserApi functionality", () => {
				...
				describe( "long-polling-tests", () => {
					...
					let thisTest = it( "plainText zosh8.esy.es, 240sec timeout", () => {
						let pageRequest: ioDatatypes.IPageRequest = {
							url: 'http://zosh8.esy.es/',
							renderType: 'plainText',
							renderSettings: {
								pdfOptions: {
									border: null,
						...
						return browserApi.requestSingle( pageRequest, { requestOptions: { timeout: 260000 } } )
							.then( ( pjscResponse ) => {
								if ( pjscResponse.content.data.indexOf( "Вітаємо" ) >= 0 ) {
									return Promise.resolve();
								}
								return Promise.reject( log.error( "example.com content should contain the word 'example'", { pjscResponse } ) );
							} )
